...
function TdmCliente.Deletar(ID: integer): Boolean;
var
  sdsDeletar: TSimpleDataSet;
begin
  sdsDeletar := TSimpleDataSet.Create(Nil);
  try
    sdsDeletar.Connection := DMConexao.DMConexao;
    sdsDeletar.DataSet.CommandText := 'DELETE FROM CLIENTE WHERE ID = :ID';
    sdsDeletar.DataSet.Params.ParamByName('ID').AsInteger := ID;
    try
      sdsDeletar.Execute;
      Result := True;
    except
